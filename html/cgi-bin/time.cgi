...
#
# $Id: time.cgi 5392 2011-02-01 20:41:22Z owes $
#
...
sub buildConfFile {
    ...
    foreach my $itf (@ITFs) {
        ...
        while ( $icount > 0 ) {
            print FILE "restrict " . $netsettings{"${itf}_${icount}_NETADDRESS"} . " mask " . $netsettings{"${itf}_${icount}_NETMASK"} . " nomodify notrap\n" if ( $netsettings{"${itf}_${icount}_NETADDRESS"} );
            $icount--;
        }
    }
