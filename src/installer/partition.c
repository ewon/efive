 ...
 *
 * $Id: partition.c 5789 2011-08-15 08:50:38Z owes $
 *
 */
...
int make_partitions(char *dev, char *dev2, long int disk_size, int part_options, long int *swap_file)
{
    ...
    /*
        Reduce disk size by 1MiB to reduce risk of partition errors caused by wrong disk size informations from /sys/block/DEVICE/size
    */
    disk_size -= 1;
...
#define FORMAT_FSTAB  "%-14s %-14s %-10s %-20s %-5s %-5s\n"
    ...
    /*  No special need to read and test for partitions.
        They are created my make_partitions() or by the user in case of manual partitioning.
    ...
    */
    ...
    for (i = 0; i < IPCOP_PARTITIONS; i++) {
        ...
        snprintf(command, STRING_SIZE, "/usr/bin/mke2fs -L %s -q -j %s", partition_label[pindex], devname);
        if (mysystem(command)) {
            newtPopWindow();
            switch (i) {
...
static int create_initramfs(void)
{
    ...
    snprintf(bigstring, STRING_SIZE, "chroot /harddisk /sbin/depmod -a %s", utsbuf.release);
...
static int make_bootable(char *dev, char *dev2, int part_options)
{
...
#if defined (__i386__)
    snprintf(bigstring, STRING_SIZE, "/bin/sed -i -e 's+KVER+%s+g' ", utsbuf.release);
    ...
    if (newworld) {
        ...
        snprintf(command, STRING_SIZE, "/bin/sed -i -e 's+KVER+%s+g' /harddisk/etc/yaboot.conf", utsbuf.release);
        mysystem(command);
    ...
    snprintf(command, STRING_SIZE, "/bin/sed -i -e 's+KVER+%s+g' /harddisk/etc/silo.conf", utsbuf.release);
    mysystem(command);
