 ...
 *
 * $Id: hardware.c 5810 2011-08-18 14:32:35Z owes $
 * 
 */
...
void scan_hardware(int flag_i_s, int nopcmcia, int nousb, int noscsi, int manualmodule)
{
    char command[STRING_SIZE];
    newtComponent form;
    newtComponent text;
    ...
    for (pcidev=pciacc->devices; pcidev; pcidev = pcidev->next) {
        ...
        if (type == none) {
            ...
            snprintf(command, STRING_SIZE,
                     "/bin/grep \"%04x 0x0000%04x\" /lib/modules/%s/modules.pcimap",
