 ...
 *
 * $Id: setaliases.c 5812 2011-08-19 07:03:15Z owes $
 *
 */
    ...
    while (fgets(s, STRING_SIZE, file) != NULL) {
        ...
        if (netmask == NULL || ( (!VALID_IP(netmask)) && (!VALID_SHORT_MASK(netmask)))) {
            ...
            snprintf(command, STRING_SIZE - 1,
                     ...
                     aliasip, ipcop_ethernet.device[RED][1], ipcop_ethernet.device[RED][1]);
            verbose_printf(2, "Add alias %s/%s\n", aliasip, ipcop_ethernet.netmask[RED][1]);
        }
        else {
            snprintf(command, STRING_SIZE - 1,
                     "/sbin/ip addr add %s/%s dev %s label %s:alias",
                     aliasip, netmask, ipcop_ethernet.device[RED][1], ipcop_ethernet.device[RED][1]);
            verbose_printf(2, "Add alias %s/%s\n", aliasip, netmask);
        }
        safe_system(command);
        memset(command, 0, STRING_SIZE);
